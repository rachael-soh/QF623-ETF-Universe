...
def calculate_screening_metrics(price_df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    for ticker, grp in price_df.groupby("ticker"):
        ...
        actual_obs = len(grp)
        missing_pct = (
            max(0.0, (expected_obs - actual_obs) / expected_obs)
            if expected_obs > 0
            else 1.0
        )
        ...
        # ── Dollar volume ─────────────────────────────────────────────────────
        dv = pd.to_numeric(
            grp.get("dollar_volume", pd.Series(dtype=float)), errors="coerce"
        )
        ...
        rdv = pd.to_numeric(
            recent_grp.get("dollar_volume", pd.Series(dtype=float)), errors="coerce"
        )
        ...
        vol_series = pd.to_numeric(
            grp.get("volume", pd.Series(dtype=float)), errors="coerce"
        )
        ...
        records.append(
            {
                ...
                "annualized_volatility": (
                    round(ann_vol, 6) if pd.notna(ann_vol) else np.nan
                ),
